...
class FakePostHogService {
  ...
  captureException(error: Error, userId: string | undefined, properties: Record<string, any>) {
...
describe('PostHogExceptionFilter', () => {
  ...
  describe('query parameter redaction', () => {
    ...
    it.each([
      ...
      const captured = runFilter(new Error('boom'), makeRequest({ query: { [name]: value } }))
    ...
    it('keeps ordinary parameters, which are the ones worth debugging with', () => {
      ...
      expect(captured.properties.queryParams).toEqual({ page: '2', sort: 'createdAt' })
    })
    ...
    it('redacts inside nested query values', () => {
      ...
      expect(JSON.stringify(captured.properties)).not.toContain('nested_secret_value')
    })
  })
  ...
  describe('exception payload bounds', () => {
    it('does not walk an arbitrarily deep object', () => {
      ...
      expect(JSON.stringify(captured.properties)).not.toContain('deep_secret_value')
    })
    ...
    it('redacts credential-looking properties on the exception itself', () => {
      ...
      expect(JSON.stringify(captured.properties.exceptionJson)).not.toContain('live_secret_value')
    })

    it('still reports enough to identify the failure', () => {
      const captured = runFilter(new BadRequestException('Token is required'), makeRequest())
