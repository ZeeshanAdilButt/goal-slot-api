...
describe('ExpoPushReminderChannel', () => {
    ...
    const findMany = jest.fn().mockResolvedValue([
      {
        ...
        userId: 'user-1',
        kind: 'EXPO',
        expoToken: 'ExponentPushToken[dead]',
      },
    ]);
  ...
  it('does not set subscriptionGone when other subscriptions remain', async () => {
    const findMany = jest.fn().mockResolvedValue([
      ...
      },
      {
        ...
        userId: 'user-1',
        kind: 'EXPO',
        expoToken: 'ExponentPushToken[alive]',
      },
    ]);
    sendPushNotificationsAsync.mockResolvedValue([
      {
    ...
    expect(prisma.pushSubscription.delete).not.toHaveBeenCalledWith({
      where: { id: 'sub-2' },
    });
    expect(result).toEqual({ ok: true });
  });
  ...
  it('catches an SDK throw and returns ok:false instead of propagating', async () => {
    ...
    sendPushNotificationsAsync.mockRejectedValue(
      new Error('Expo API unreachable'),
    );
    const { channel } = buildChannel({ findMany });
