...
describe('ConversationGateSecretGuard', () => {
  ...
  it('rejects a request with no Authorization header', () => {
    const guard = new ConversationGateSecretGuard(new MessagingConfigService(CONFIGURED));
  ...
  it('rejects a non-Bearer scheme even if the value matches', () => {
    ...
    expect(() => guard.canActivate(contextWithAuthHeader('Basic shared-secret'))).toThrow(
      UnauthorizedException,
    );
  });
  ...
  it('rejects a GoalSlot user JWT the same as any other wrong credential', () => {
    ...
    const guard = new ConversationGateSecretGuard(new MessagingConfigService(CONFIGURED));
    const looksLikeAJwt = 'eyJhbGciOiJIUzI1NiJ9.eyJzdWIiOiJ1c2VyXzEifQ.signature';

    expect(() => guard.canActivate(contextWithAuthHeader(`Bearer ${looksLikeAJwt}`))).toThrow(
      UnauthorizedException,
    );
  });
  ...
  it('allows a request with the exact configured secret', () => {
    ...
    expect(guard.canActivate(contextWithAuthHeader('Bearer shared-secret'))).toBe(true);
  });
});
