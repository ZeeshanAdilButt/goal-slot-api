...
- \`CREATE_TASK\`             payload: { title, goalId?, scheduleBlockId?, dueDate? }
                              ...
                              Do NOT use CREATE_TASK when the sentence names a destination page as "notes"/"note"/"page" (e.g. "add X to my Y notes", "put this on my Y page") — check the "## The user's Notes pages" list first; see APPEND_NOTE_CONTENT below, including its own no-match/ask rule. Only treat it as a plain task when the sentence names no page at all ("add a task to call the bank", "remind me to buy milk").
