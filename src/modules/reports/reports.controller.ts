...
export class ReportsController {
  ...
  async getGoalsProgress(@Request() req: AuthenticatedRequest) {
    return this.reportsService.getGoalProgress(req.user.sub);
  }

  @Get('goal-progress')
  @ApiOperation({ summary: 'Get all goals progress (alias)' })
  async getGoalProgress(@Request() req: AuthenticatedRequest) {
    return this.reportsService.getGoalProgress(req.user.sub);
  }
  ...
  @ApiOperation({
    ...
    description:
      'Returns a detailed report showing every time entry with timestamps, similar to a spreadsheet. Includes daily totals. Perfect for invoicing, mentor reports, and student assignment updates.',
  })
  ...
  @ApiOperation({
    ...
    description:
      'Returns a compact summary view showing total accumulated hours by goal, task, category, or date. No individual timestamps shown.',
  })
  ...
  @ApiOperation({
    ...
    description:
      'Returns report showing total hours per day with merged task names.',
  })
  ...
  @ApiOperation({
    ...
    description:
      'Returns report showing hours logged per schedule block, with day-by-day breakdown. Shows how well you stuck to your schedule.',
  })
  ...
  async getFilterableGoals(@Request() req: AuthenticatedRequest) {
  ...
  @ApiOperation({
    ...
    description:
      'Generate downloadable report in CSV, PDF, or JSON format. Supports detailed and summary views with customizable filters, billable hours calculations, and client/project information for invoicing.',
  })
  ...
  async exportReport(
    ...
    const exportData = await this.reportsService.generateExport(
      req.user.sub,
      filters,
    );
    ...
    if (filters.format === ExportFormat.CSV && typeof exportData === 'string') {
      ...
      res.setHeader(
        'Content-Disposition',
        `attachment; filename="time-report-${filters.startDate}-to-${filters.endDate}.csv"`,
      );
