...
export class CoachJournalController {
  ...
  @Get()
  @ApiOperation({
    summary: 'List journal entries for the user (default last 60 days)',
  })
  async list(
  ...
  @ApiOperation({
    summary: 'Set the HTML content of an entry (upsert if missing)',
  })
  async updateContent(
    @Request() req: AuthenticatedRequest,
    @Param('date') date: string,
  ...
  @ApiOperation({
    summary: 'Set the mood/energy of an entry (upsert if missing)',
  })
  async updateMood(
    @Request() req: AuthenticatedRequest,
    @Param('date') date: string,
