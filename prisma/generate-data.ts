...
const goalTemplates = [
  ...
  {
    title: "Complete 500 DSA Problems",
    category: "WORK",
    targetHours: 300,
    color: "#10B981",
  },
...
const taskTemplates = [
  ...
  {
    title: "Build Todo app with Next.js",
    category: "LEARNING",
    estimatedMinutes: 180,
  },
  {
    title: "Read TypeScript handbook chapter",
    category: "LEARNING",
    estimatedMinutes: 60,
  },
  {
  ...
  {
    title: "Solve Dynamic Programming (2)",
    category: "DSA",
    estimatedMinutes: 180,
  },
...
async function main() {
  ...
  console.log(
    `📋 Available categories: ${Array.from(categoryValues).join(", ")}`,
  );
  ...
  const validGoalTemplates = goalTemplates.filter((t) =>
    categoryValues.has(t.category),
  );
  if (validGoalTemplates.length === 0) {
    console.warn(
      "⚠️  No valid goal templates found (no matching categories). Skipping goals.",
    );
  } else {
  ...
  const validScheduleTemplates = scheduleTemplates.filter((t) =>
    categoryValues.has(t.category),
  );
  if (validScheduleTemplates.length === 0) {
    console.warn(
      "⚠️  No valid schedule templates found (no matching categories). Skipping schedule blocks.",
    );
  } else {
        ...
        Math.random() > 0.5
          ? randomElement(
              goals.filter(
                (g) =>
                  g.category &&
                  (g.category === "LEARNING" || g.category === "WORK"),
              ),
            )
          : null;
  ...
  const validTaskTemplates = taskTemplates.filter((t) =>
    categoryValues.has(t.category),
  );
  if (validTaskTemplates.length === 0) {
    console.warn(
      "⚠️  No valid task templates found (no matching categories). Skipping tasks.",
    );
  } else {
    ...
    for (let day = 0; day < 365; day++) {
      ...
      for (let i = 0; i < tasksPerDay; i++) {
        ...
        const scheduleBlock =
          Math.random() > 0.7 ? randomElement(scheduleBlocks) : null;

        // Determine status based on date
        let status: TaskStatus;
        const daysSinceCreation = Math.floor(
          (Date.now() - taskDate.getTime()) / (1000 * 60 * 60 * 24),
        );
          ...
          data: {
            ...
            actualMinutes:
              status === TaskStatus.DONE
                ? estimatedMinutes + randomInt(-20, 40)
                : null,
            completedAt:
              status === TaskStatus.DONE
                ? addDays(taskDate, randomInt(0, 7))
                : null,
  ...
  for (let day = 0; day < 365; day++) {
    ...
    for (let i = 0; i < entriesPerDay; i++) {
      ...
      const scheduleBlock =
        Math.random() > 0.6
          ? randomElement(
              scheduleBlocks.filter((b) => b.dayOfWeek === dayOfWeek),
            )
          : null;
      ...
      const source =
        Math.random() > 0.2
          ? TimeEntrySource.TRACKER
          : TimeEntrySource.COMPLETION;

      // Create time entry
      const taskName = linkedTask
        ? linkedTask.title
        : validTaskTemplates.length > 0
          ? randomElement(validTaskTemplates).title
          : "General Task";
    ...
    if ((day + 1) % 50 === 0) {
      console.log(
        `  Created ${entryCount} time entries for ${day + 1} days...`,
      );
    }
  }
  ...
  console.log(
    `   - Total hours logged: ${(timeEntries.reduce((sum, e) => sum + e.duration, 0) / 60).toFixed(2)}`,
  );
}
